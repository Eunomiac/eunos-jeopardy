...
- **Player Experience**: Modal disappears for selected player (❌ ISSUE: modal should only disappear when player is marked wrong, not when selected)
...
4. **Modal disappears only for the wrong player** ❌ Currently wrong (modal disappears for all players)
5. **Buzzer queue is cleared** of current attempt ✅ Currently correct
6. **Host can continue** to accept buzzes from other players ❌ Currently wrong

### **Clue Completion Conditions**

A clue should only be marked as completed (`focused_clue_id = null`) when:
...
- Set `focused_clue_id = null` (complete clue)
...
- Update player score (-clue value)
- Keep `focused_clue_id = current clue` (clue stays active)
...
- Set `is_buzzer_locked = false` (unlock for remaining players)
...
- **Timeout behavior**:
  ...
  - Show host notification: "Clue timed out - no players scored"
- Host interface should show a countdown timer to indicate remaining time

### **Player Interface Changes Required**

#### **1. Modal Persistence**
- Modal should NOT disappear when player is selected by host
- Modal should disappear for individual players when THEY are marked wrong
- Modal should disappear for ALL players when clue is completed (correct answer, clue timeout, or all players wrong)
...
1. Host reveals clue, unlocks buzzer. Timeout starts (e.g., 5 seconds)
2. All players buzz and are marked wrong
3. **Expected**: Clue should be completed (no one left to answer)
4. Host should have option to give correct answer (perhaps display the correct answer via an alert, which should remain until the host does something else)
...
5. **Expected**: Clue automatically completed, all modals disappear, no score changes, host sees timeout notification
...
### **Player Interface**
- `src/components/players/PlayerDashboard.tsx` - Update modal persistence logic
- `src/components/players/PlayerBuzzer.tsx` - Add "LOCKED OUT" state
